...
public class MutualExclusion {
	...
	public boolean doMutexRequest(Message message, byte[] updates) throws RemoteException {
		...
		// start MutualExclusion algorithm
			...
			List<Message> unikPeer = removeDuplicatePeersBeforeVoting();
			
			// multicast the message to activenodes (hint: use multicastMessage)
			multicastMessage(message, unikPeer);
			
			// check that all replicas have replied (permission) - areAllMessagesReturned(int numvoters)?
			int numvoters = unikPeer.size();
	...
	public void doDecisionAlgorithm(Message message, List<Message> queue, int condition) throws RemoteException {
		...
		Message recMessage = node.getMessage();
		...
		switch(condition) {
			...
			case 2: {
				...
				int recClock = recMessage.getClock();
				
				// compare clocks, the lowest wins
				if(sendClock < recClock) {
					//sender wins
					NodeInterface stub = Util.getProcessStub(procName, port);
					message.setAcknowledged(true);
					stub.onMutexAcknowledgementReceived(message);
				}
				else if (sendClock == recClock) {
					// if clocks are the same, compare nodeIDs, the lowest wins
					if(message.getNodeID().compareTo(recMessage.getNodeID()) < 0) {
	...
	public void multicastReleaseLocks(Set<Message> activenodes) {
		...
		for(Message m : activenodes) {
			...
			try {
				...
				logger.error("Failed to release lock on node: " + m.getNodeName(), e);
			}
		}
